...
function eventer_show_title($title = '', $id = '')
{
	...
	$title .= $passed_badge;
	$title .= ($badge != '') ? ' <span class="eventer-status-badge">' . $badge . '</span>' : '';
	return $title;
}
        ...
        $days_diff = eventer_dateDiff($event_time, $event_end_time);
				...
				if (!empty($valvalue) && $valindex != 'main') {
					foreach ($valvalue as $val) {
						...
						$time = ($time_slot != '00:00:00' && $time_slot != '') ? $slot_title : date_i18n(get_option('time_format'), strtotime($event_time)).' - '.date_i18n(get_option('time_format'), strtotime($event_end_time));
