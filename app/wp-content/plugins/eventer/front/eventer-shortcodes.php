...
class eventer_stage2_generate_shortcode
{
  ...
  function eventer_grid_style_stage2($atts)
  {
    ...
    echo '<div class="eventer eventer-dynamic-listings-main eventer-grid eventer-grid-' . esc_attr($grid_class) . ' eventer-grid-col' . esc_attr($atts['column']) . '"  data-shortcode="' . esc_attr(json_encode($atts)) . '">';
    ...
    if ($get_eventer_grid['results'] <= 0) {
      ...
      echo esc_html__('Sorry, but we did not found any event to show here.', 'eventer');
      echo '</div>';
      //return $no_event_msg;
    } else {
