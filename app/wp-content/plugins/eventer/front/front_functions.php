...
if (!function_exists('eventer_enqueue_scripts')) {
  function eventer_enqueue_scripts()
  {
    ...
    if (function_exists('is_checkout') && is_checkout() && !empty(is_wc_endpoint_url('order-received'))) {
      ...
      $localized_event_data = array('enabled_date' => get_post_meta(get_the_ID(), 'eventer_all_dates', true), 'min_date' => get_query_var('edate'), 'max_date' => date_i18n('Y,m,d', strtotime(max(get_post_meta(get_the_ID(), 'eventer_all_dates', true)))), 'optional_tickets' => get_post_meta(get_the_ID(), 'eventer_optional_tickets', true), 'dynamic_event' => get_the_ID());
    }
...
if (!function_exists('eventer_pagination')) {
  function eventer_pagination($pages = '', $paged = 1, $range = 4)
  {
    $default = '';
    if (is_tax()) {
    ...
    if (1 != $pages) {
      ...
      if ($paged != 1) {
        $pagi .= '<li><a href="' . esc_url(eventer_generate_endpoint_url('pagin', 1, $permalink, $default)) . '" title="' . __('First', 'eventer') . '">' . esc_html__('First', 'eventer') . '</a></li>';
      }
      for ($i = 1; $i <= $pages; $i++) {
        if (1 != $pages && (!($i >= $paged + $range || $i <= $paged - $range) || $pages <= $showitems)) {
          $pagi .= ($paged == $i) ? "<li class=\"active\"><span>" . $i . "</span></li>" : "<li><a href='" . esc_url(eventer_generate_endpoint_url('pagin', $i, $permalink, $default)) . "' class=\"\">" . $i . "</a></li>";
        }
      }
      if ($paged != $pages) {
        $pagi .= '<li><a href="' . esc_url(eventer_generate_endpoint_url('pagin', $pages, $permalink, $default)) . '" title="' . __('Last', 'eventer') . '">' . esc_html__('Last', 'eventer') . '</a></li>';
      }
      $pagi .= '</ul>';
    }
    return $pagi;
...
if (!function_exists('eventer_registrant_tickets')) {
  function eventer_registrant_tickets()
  {
    ...
    if ($book_type == 'woo') {
      ...
      if ($cart_status != '') {
        foreach ($woocommerce->cart->get_cart() as $key => $item) {
          ...
          if ($cart_status == $cart_product_id) {
      ...
      ob_end_clean();
      echo wp_json_encode(array('reg' => '', 'woo' => $output, 'stripe_error' => '1', 'stripe_msg' => ''));
      wp_die();
    }
    ...
    if ($book_type == 'stripe') {
      ...
      $data = generatePaymentResponse($intent);
      ...
      if ($verified_status == 'succeeded' && $verified_amount_received == ($itemPrice * 100)) {
    ...
    if (!empty($tickets)) {
      ...
      if (get_post_meta($eventer_id, 'eventer_common_ticket_count', true) != '') {
        $get_all_tickets = eventer_update_date_wise_bookings_table($eventer_id, $eventer_date . ' ' . $eventer_time, array(), 2);
        if ($get_all_tickets) {
          foreach ($get_all_tickets as $get_ticket) {
            $all_tickets = (isset($get_ticket['dynamic'])) ? $get_ticket['dynamic'] : '';
            eventer_update_date_wise_bookings_table($eventer_id, $eventer_date . ' ' . $eventer_time, array(array('id' => $all_tickets, 'number' => $total_booked)));
          }
        }
      } else {
        eventer_update_date_wise_bookings_table($eventer_id, $eventer_date . ' ' . $eventer_time, $tickets);
      }
    }
    ...
    echo wp_json_encode(array('reg' => $registration_id, 'woo' => '', 'stripe_error' => '', 'stripe_msg' => '', 'secret' => $secret, 'reg_id' => $lastid));
    wp_die();
  }
  ...
  function eventer_execute_registrant_event($first, $eventer_id = '', $date_dynamic = '', $time_dynamic = '00:00:00')
  {
    ...
    if ($time_slot_setup) {
      ...
      foreach ($time_slot_setup as $slot) {
        ...
        $time_slot_values .= '<option ' . (($time_dynamic == $slot['start'] . ':00') ? 'selected' : '') . ' value="' . esc_attr($slot['start'] . ':00') . '">' . $slot['title'] . ' ' . $slot['start'] . '-' . $slot['end'] . '</option>';
      }
...
if (!function_exists('eventer_save_events')) {
  function eventer_save_events()
  {
    ...
    if (isset($query_string['action']) && isset($query_string['id']) && isset($query_string['key']) && $query_string['key'] == 'imic_save_event') {
      ...
      $triggerOn_start = date_i18n('Y-m-d H:i', $start_dt_str);
      $triggerOn_end = date_i18n('Y-m-d H:i', $end_dt_str);
...
$default_attribs = array('name' => array(), 'class' => array(), 'maxlength' => array(), 'multiple' => array(), 'id' => array(), 'data-tprice' => array(), 'type' => array(), 'name' => array(), 'value' => array(), 'class' => array(), 'data-mprice' => array(), 'style' => array(), 'data-booked' => array(), 'data-ticketid' => array(), 'data-tooltip' => array(), 'data-identify' => array(), 'data-cart' => array(), 'data-pid' => array(), 'data-limit' => array());
...
function eventer_dynamic_ticket_area()
{
  ...
  echo json_encode(array("tickets" => do_shortcode('[eventer_tickets id="' . $event . '" date="' . $date . '" time="' . $time . '"]'), "metas" => do_shortcode('[eventer_metas id="' . $event . '" date="' . $date . '" time="' . $time . '"]'), "date_show" => $date_show, "date" => $date, "formatted" => $formatted));
  wp_die();
}
