...
function eventer_register_shortcodes()
{
					...
					if (!empty($atts['param'])) {
						...
						if (array_keys($array) !== range(0, count($array) - 1) && !empty($array)) {
							...
							foreach ($params as $param) {
								...
								$checkboxes .= ' <input data-meta="' . esc_attr($atts['meta_key']) . '" type="checkbox" ' . $required_data . ' ' . $name . ' ' . $checked . ' class="eventer_add_event_field eventer_set_class_for_add ' . esc_attr($atts['class']) . '" value="' . esc_attr($param) . '"> ' . esc_attr($param);
							}
							$field .= $checkboxes;
							$field .= '</div>';
						}
	...
	*/
	...
	function eventer_ticket_section($atts, $content = "")
	{
		...
		$params['dynamic_val'] = wp_rand(100, 99999999);
