  ...
  function eventer_add_form_fields_venue($term, $taxonomy)
  {
          ...
          if ($new_user_system_restore == 1) {
            ...
            if (!empty($tickets)) {
              foreach ($tickets as $ticket_set) {
                ...
                eventer_update_date_wise_bookings_table($registrants->eventer, $time_slot, array(array('id' => $tickets_id, 'number' => $total_tickets)), 3, 1);
              }
            }
          }
      ...
      function eventer_remove_registrant()
      {
        ...
        if ($reg_id != '' && $reg_email != '') {
          ...
          if ($reg_details) {
            ...
            //There should be the code to update number of tickets for deleted booking
          }
	...
	*/
      function eventer_get_booked_tickets()
      {
        ...
        echo json_encode(array('tickets' => $updated_tickets_new, 'featured' => $default_featured));
        die();
      }
      add_action('wp_ajax_eventer_get_booked_tickets', 'eventer_get_booked_tickets');
    }

    function eventer_update_date_wise_bookings_table($event, $date, $tickets = array(), $insert = 1, $update = 1)
    {
      if (is_array($event) || !is_numeric($event)) return;
      ...
      $saved_ticket = $wpdb->get_results("SELECT * FROM $table_name_tickets WHERE event = $event AND date = '$booked_date' AND type = '$ticket_type'", ARRAY_A);
      ...
      if (empty($saved_ticket) && $insert <= 2 && $tickets_created) {
        foreach ($tickets_created as $ticket_new) {
          $ticket_pid = (isset($ticket_new['pid'])) ? $ticket_new['pid'] : wp_rand(100, 9999);
          $woo_payment = eventer_get_settings('eventer_enable_woocommerce_ticketing');
          if ($woo_payment == 'on') {
          ...
          if (empty($saved_ticket_verify)) {
            $wpdb->query($wpdb->prepare("INSERT INTO $table_name_tickets ( dynamic, pid, event, name, date, type, tickets, price, restricts, featured, label, enabled ) VALUES ( %d, %d, %d, %s, %s, %s, %s, %f, %s, %s, %s, %s ) ", $dynamic_ids, $ticket_pid, $original_event, stripslashes($ticket_name), $booked_date, $ticket_type, $ticket_number, $ticket_price, $ticket_restrict, $ticket_featured, $ticket_badge, $ticket_enabled));
          }
        }
      } else if ($insert == 2) {
        $saved_ticket = $wpdb->get_results("SELECT * FROM $table_name_tickets WHERE event = $event AND date = '$booked_date' AND type = '$ticket_type'", ARRAY_A);
        ...
        foreach ($tickets as $save_ticket) {
          ...
          if ($dynamic_ids != '') {
            ...
            $saved_ticket = $wpdb->get_row("SELECT * FROM $table_name_tickets WHERE event = $original_event AND date = '$booked_date' AND name = '$validate_ticket_name' AND type = '$ticket_type'");
          }
          ...
          if ($saved_ticket) {
            ...
            if ($dynamic_ids) {
              $wpdb->update($table_name_tickets, array('name' => stripslashes($ticket_name), 'type' => $ticket_type, 'tickets' => $ticket_number, 'price' => $ticket_price, 'pid' => $ticket_pid, 'restricts' => $ticket_restrict, 'featured' => $ticket_featured, 'label' => $ticket_badge, 'enabled' => $ticket_enabled), array('event' => $original_event, 'date' => $booked_date, 'dynamic' => $dynamic_ids, 'type' => $ticket_type), array('%s', '%s', '%s', '%f', '%d', '%s', '%s', '%s', '%s'), array('%d', '%s', '%d'));
            } else {
              $wpdb->update($table_name_tickets, array('name' => stripslashes($ticket_name), 'type' => $ticket_type, 'tickets' => $ticket_number, 'price' => $ticket_price, 'pid' => $ticket_pid, 'restricts' => $ticket_restrict, 'featured' => $ticket_featured, 'label' => $ticket_badge, 'enabled' => $ticket_enabled), array('event' => $original_event, 'date' => $booked_date, 'name' => $validate_ticket_name, 'type' => $ticket_type), array('%s', '%s', '%s', '%f', '%d', '%s', '%s', '%s', '%s'), array('%d', '%s', '%d'));
            }
          }
        }
      }
      ...
      function eventer_update_booked_tickets()
      {
        ...
        if (!empty($tickets_new) && empty($setup_tickets_new)) {
          foreach ($tickets_new as $ticket_get) {
            if (isset($ticket_get['pid']) && $ticket_get['pid'] == '' && $ticket_get['name'] != '') {
              ...
              eventer_update_date_wise_bookings_table($eventer_id, $booked_date . ' ' . $booked_time, array($ticket_get), 2);
            }
        ...
        if ($woocommerce_events != 'on') {
          ...
          foreach ($export_query as $Result) {
            ...
            foreach ($order->get_items() as $item_key => $item_values) :
              ...
              $data_set['Name'] = get_post_meta($woo_order, '_billing_first_name', true) . ' ' . get_post_meta($order_id, '_billing_last_name', true);
