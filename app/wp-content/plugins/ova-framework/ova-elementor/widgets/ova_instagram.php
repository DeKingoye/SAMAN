...
class ova_instagram extends Widget_Base {
	...
	protected function render() {
		...
		// var_dump(wp_json_encode( $instagram_slide));die;
		...
		<?php if ($access_token != '') {
			...
			if( is_array( $result ) && ! is_wp_error( $result ) ) {
				...
				$user_name    = isset( $obj['data']['0']['user']['username'] ) ? $obj['data']['0']['user']['username'] : '' ;
					...
					<div class="slide owl-carousel owl-theme" <?php echo $this->get_render_attribute_string( 'slide' ) ?> >
						...
						if( isset($obj['data']) && $obj['data'] ){
							foreach ($obj['data'] as $post){ 
								...
								<?php if( in_array($settings['hashtag'], $hashtag) and $settings['hashtag'] != '' ) { ?>
									<div class="item" >
								...
								<?php  } if( $settings['hashtag'] == '' )  { ?>
									<div class="item" >
										<div class="image" style="background-image: url('<?php echo esc_attr($pic_src) ?>'); height: <?php echo esc_attr( $settings['height'] ).'px' ?>"></div>
										<div class="overlay" >
											<a href="<?php echo esc_attr($pic_link) ?>" rel="nofollow" target="_blank"><i class="linkToIns fab fa-instagram" ></i></a>
										</div>
									</div>
								<?php } 
							}
