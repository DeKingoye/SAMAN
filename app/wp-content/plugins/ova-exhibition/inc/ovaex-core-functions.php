...
function ovaex_pagination_plugin($ovaex_query = null) {
	...
	if($ovaex_query != null){
		if( $ovaex_query->max_num_pages <= 1 )
			return;	
	...
	echo wp_kses( __( '<div class="blog_pagination"><ul class="pagination">','ovaex' ), true ) . "\n";
	...
	if ( ! in_array( 1, $links ) ) {
		...
		if ( ! in_array( 2, $links ) )
			echo wp_kses( __('<li><span class="pagi_dots">...</span></li>', 'ovaex' ) , true);
	}
	...
	echo wp_kses( __( '</ul></div>', 'ovaex' ), true ) . "\n";
